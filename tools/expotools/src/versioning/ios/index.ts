...
async function generateReactNativePodspecsAsync(
  ...
  const headerDirs = getCppLibrariesToVersion().map(lib => lib.customHeaderDir || lib.libName);
  ...
  for (const podspecFile of podspecFiles) {
    ...
    const podspecSource = (await fs.readFile(podspecFile, 'utf8'))
      .replace(new RegExp(`"(${podNames.join('|')})((?:\/[^"]+)?)"`, 'g'), `"${versionName}$1$2"`)
      .replace(new RegExp(`\.header_dir(\s*)=(\s*)(["'])(${headerDirs.join('|')})(["'])`, 'g'), `.header_dir$1=$2$3${versionName}$4$5`)
      .replace(/\.exclude_files(\s*)=(\s*)("|')(SampleCxxModule\.\*)("|')/g, `.exclude_files$1=$2$3${versionName}$4$5`) // for React-cxxreact podspec
      .replace(/([\{\,])RCT/g, `$1${versionName}RCT`)
      .replace(/(Headers\/Private\/)(React-Core)/g, `$1${versionName}$2`);
...
async function generateYogaPodspecAsync(
  ...
  const versionedLibraryName = getVersionedLibraryName('Yoga', versionName);
...
export async function addVersionAsync(versionNumber: string) {
  ...
  console.log(chalk.yellow('\n>'), 'Removing any `filename--` files from the new pod ...');
